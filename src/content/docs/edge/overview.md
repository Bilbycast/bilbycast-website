...
| **MXL** | Yes | Yes | EBU / Linux Foundation Media eXchange Layer same-host shared-memory bus (`mxl_video` / `mxl_audio` / `mxl_anc`), PTP-required; off-by-default `mxl` feature. See [MXL](/edge/mxl/) |
...
| **`test_pattern` / `media_player` / `replay`** | Yes | — | Synthetic + local input types: colour-bar/tone generator, local-file playlist player (TS / MP4 / image), and recorded-clip replay. `media_player`/`replay` are default-on |
| **`mosaic`** | Yes | — | Multiviewer canvas — composites N node-local inputs into one frame (up to 1920×1080, 64 tiles) and republishes it as an ordinary MPEG-TS flow source. Off-by-default `multiviewer` feature, on in all three release artefacts; a build with no `video-encoder-*` feature does not advertise `mv-compositor`. See [Multiviewer](/edge/multiviewer/) |
...
- **Multi-path bonding** — A third bonding option for N ≥ 2 heterogeneous UDP / QUIC legs (5G + Starlink + fibre), distinct from SRT socket groups and RIST 2022-7. The default capacity-aware `adaptive` scheduler discovers each leg's usable bandwidth and splits the stream to track it while duplicating H.264/HEVC IDR frames across the two best legs; the bond layer itself is transport-agnostic and can carry SRT, RTMP, RTSP, ST 2110, or any other inner protocol — see [Multi-Path Bonding](/edge/bonding/)
...
- **Node Bus — cross-flow ES routing** — The PID bus is **node-wide**: any flow's assembly can pull elementary streams from inputs owned by sibling flows on the same edge, with a master-clock identity preflight that refuses cross-PTP-domain / cross-PCR-PLL combinations before the WS round-trip. One edge becomes a full distribution matrix — fan one input's program out to N flows, change source per-output without dropping any other flow. The manager UI surfaces this as the [Node Bus Matrix](/manager/node-bus/) — a three-pane crosspoint authoring surface (Sources / Matrix / Inspector) with click-to-wire + drag-and-drop, pending-state diffing, and salvo export to a Switcher preset
...
- **Multiviewer (mosaic wall)** — A node-local compositor lays N of the node's own inputs out on a single canvas (up to 1920×1080, 64 tiles) and publishes the result as an ordinary MPEG-TS input, so a monitoring wall is just another flow source and reaches any output the edge already has — including a tile whose source is a small proxy arriving over SRT from another site. Behind the off-by-default `multiviewer` Cargo feature (on in all three release artefacts) and gated on a compiled-in video encoder, since the canvas has to be encoded to ride the TS flow bus. Walls are authored and deployed from the manager's [Multiviewer Walls](/manager/multiviewer/) — see [Multiviewer](/edge/multiviewer/)
...
- **Cross-node egress alignment** — Two edges forwarding the same contribution feed can be placed on one shared timeline, so they emit the same content at the same wall instant and a downstream switcher can cut between them without a timing discontinuity. Configured per UDP/RTP output as an `epoch_lock` block; the shared timeline is minted by the manager, since no single node can derive one that does not carry its own ingest latency. Scope is deliberately narrow — single-input, single-program, non-transcoded forwarding with the source clock passed through. See [Aligned Output](/manager/aligned-output/)
- **Egress + ingress de-jitter** — Per-output `egress_pacing` (`forward` / `pcr` / `servo`) chooses when datagrams hit the wire, with an optional servo cushion; per-input `ingress_dejitter_ms` recovers the source rate from inter-PCR observations and releases packets at that rate, so analysers, the PCR PLL, the PID bus and every downstream output see a smooth cadence regardless of network jitter. Both are ordinary manager-configurable fields — see [Configuration](/edge/configuration/#egress-pacing)
