...
The path is picked at spawn time from two gates: the `BILBYCAST_ENABLE_TXTIME` env var **and** the output's wire-pacing class. When the env var is unset (the default), the emitter stays on `clock_nanosleep` regardless of what the kernel reports. When set to `1`, `SO_TXTIME` activates **only** for an ETF-eligible output — ST 2110 uncompressed essence (`WirePacingClass::EtfEligible`) — and only if the `SO_TXTIME` probe then succeeds; otherwise it falls back to `clock_nanosleep`. Every compressed / Lossless output — meaning every UDP / RTP / SMPTE 302M TS output — stays on `clock_nanosleep` even with `BILBYCAST_ENABLE_TXTIME=1` set and a successful probe, because a bursty compressed source gains nothing from per-packet kernel launch times. The active tier shows on every output's stats as `wire_pacing_tier`.
