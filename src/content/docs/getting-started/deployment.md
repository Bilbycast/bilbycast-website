...
|---|---|---|
| `ffmpeg` | (1) Per-flow thumbnail generation (`thumbnail: true` on a flow) — the edge spawns `ffmpeg` every ~10 s, pipes ~3 s of buffered TS into it, and reads back a 320×180 JPEG. (2) Phase B compressed-audio egress for non-AAC codecs (`audio_encode` block with Opus, MP2, or AC-3 on RTMP, HLS, or WebRTC outputs). **Not required for AAC codecs** — the default `fdk-aac` feature encodes AAC-LC, HE-AAC v1/v2 in-process via Fraunhofer FDK AAC. See [Audio Gateway](/edge/audio-gateway/) for the full pipeline. | Thumbnails are detected as unavailable at startup; flows with `thumbnail: true` still run normally but no preview images appear in the manager UI. Outputs with `audio_encode` using non-AAC codecs refuse to start without ffmpeg and emit a Critical `audio_encode` event — AAC codec outputs work without ffmpeg when the `fdk-aac` feature is enabled (default). Outputs without `audio_encode` keep working without ffmpeg installed. |
