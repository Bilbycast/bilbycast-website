...
|---|---|---|
| `ffmpeg` | (1) Per-flow thumbnail generation (`thumbnail: true` on a flow) — the edge spawns `ffmpeg` every ~10 s, pipes ~3 s of buffered TS into it, and reads back a 320×180 JPEG. (2) Phase B compressed-audio egress (`audio_encode` block on an RTMP, HLS, or WebRTC output) — RTMP and WebRTC run one persistent ffmpeg per encoded output, HLS forks ffmpeg per segment as a TS-in/TS-out remuxer. Supported output codecs: AAC-LC, HE-AAC v1/v2, Opus, MP2, AC-3. See [Audio Gateway](/edge/audio-gateway/) for the full pipeline. | Thumbnails are detected as unavailable at startup; flows with `thumbnail: true` still run normally but no preview images appear in the manager UI. Outputs with `audio_encode` refuse to start and emit a Critical `audio_encode` event — outputs without `audio_encode` keep working without ffmpeg installed. |
