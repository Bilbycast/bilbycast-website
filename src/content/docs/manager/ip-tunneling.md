...
- **Opaque relay**: the relay forwards traffic by tunnel UUID and holds no key that can decrypt or forge it. Opaque is not the same as unauthenticated — see the next bullet.
- **Per-tunnel bind authentication**: the manager mints a random bind secret per tunnel, stores it encrypted at rest, and registers per-direction bind tokens on the relay with `authorize_tunnel` (HMAC-SHA256 over `{tunnel_id}:{direction}`, hex-encoded, compared in constant time). Both carriers enforce it — the QUIC bind and the native-UDP register — so a bind that presents no token, or the token for the other direction, is refused. Set `require_bind_auth: true` on the relay to *also* fail closed for tunnels the manager has never authorized; the default (`false`) fails open for those only. Rotate a tunnel's secret with `POST /api/v1/tunnels/{id}/rotate-bind-secret`.
- **Tunnel ID security**: tunnel IDs are random 128-bit UUIDs. Knowledge of a UUID alone does not grant access — a leaked UUID still cannot claim an endpoint without the matching bind token.
- **Relay access controls**: the relay's REST API takes an optional Bearer `api_token` (32–128 chars) that guards every endpoint except `/health`, with the destructive delete routes refusing to run at all until it is configured. QUIC connections and native-UDP sessions are additionally capped per source IP.
